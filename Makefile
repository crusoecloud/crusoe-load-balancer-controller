...
IMG ?= pphelan001/controller:latest
...
.PHONY: dev
dev: test build-deps ## Runs a build-deps, test, lint
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name crusoe-load-balancer-controller-builder
	$(CONTAINER_TOOL) buildx use crusoe-load-balancer-controller-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm crusoe-load-balancer-controller-builder
	rm Dockerfile.cross
